...
while t < SIM_TIME
    ...
    %Disturbance
    position_pert(1) = sin(t*10)*.1;
    position_pert(2) = cos(t*10)*.1;
    ...
    % desired_position_unmodified = desired_position;
    ...
    % if rd > restriction_radius*increment_v
    ...
    %     rd = sqrt(position(1)^2 + position(2)^2 + position(3)^2);
    % end
    ...
    % for ac = 1:3
    %     if abs(velocity_error(ac)) > 0.4
    %         velocity_error(ac) = 0.4*sign(velocity_error(ac));
    %     end
    % end
    % if position
    % for ab = 1:3
    %     if
    %
    %        % position_error(1) = position(1) - (restriction_radius + 0.1)*sin(beta)*cos(theta);
    %        % position_error(2) = position(2) - (restriction_radius + 0.1)*sin(beta)*sin(theta);
    %        % position_error(3) = position(3) - (restriction_radius + 0.1)*cos(beta);
    %     end
    % end
    ...
    %   for ab = 1:3
    %     if abs(desired_position)
    %         position_error(ab) = 0.1*sign(position_error(ab));
    %     end
    %   end
    % end
    ...
    for j = 1:3
        ...
        % trans_kp_dot(j) = lambda(1)*abs(position_error(j)) + lambda(2)*(trans_kp_o(j) - trans_kp(j));
...
view([45,40])
% view([-266.321,49.2774])
grid off;
print('./plots/restricted_control_pert/3D_Trajectory', '-depsc2')

% figure;
% set(gcf,"Color",[1,1,1])
% set(gcf,"Position",[73,220,1848,647])
% set(gcf,"Renderer","painters")
% plot(time, plot_attitude(1, :), 'k','LineWidth',5);
% hold on
% plot(time, plot_attitude(2, :), 'r','LineWidth',5);
% plot(time, plot_attitude(3, :), 'g','LineWidth',5);
% plot(time, plot_attitude(4, :), 'b','LineWidth',5);
...
%     'Position', [0.912578137732572,0.038987628164395,0.069172728863859,0.971097386638753]);
% set(gca,"FontSize", 30);
% xlabel('Time [s]', 'FontSize',40);
% ylabel('Attitude Quaternion', 'FontSize',40);
% box("off")
% grid off;
% print('./plots/restricted_control_pert/Attitude', '-depsc2')
% hold off


% figure;
% set(gcf,"Color",[1,1,1])
% set(gcf,"Position",[100,124,1848,621])
% set(gcf,"Renderer","painters")
% plot(time,plot_desired_position(1, :), 'r','LineWidth',5);
% hold on
% plot(time,plot_position(1, :), 'r','LineWidth',0.5, 'Marker','o','MarkerEdgeColor', 'k' ,'MarkerSize', 12,'MarkerFaceColor',[0.00,0.45,0.74],'MarkerIndices',marker_space);
% % plot(time,plot_position_adaptive(1, :), 'b','LineWidth',5);
% % plot(time,plot_desired_position_adaptive(1, :), 'b-.','LineWidth',5);
% ylim([-1 1])
% xlim([0 SIM_TIME])
% legend('$p_x$', '$p_{xr}$', ...
%     'FontSize',48, 'interpreter', 'latex', ...
...
% grid off;
% box("off")
% print('./plots/restricted_control_pert/Position1', '-depsc2')
% hold off
...
% set(gcf,"Renderer","painters")
% plot(time,plot_desired_position(2, :), 'g','LineWidth',5);
% hold on
% plot(time,plot_position(2, :), 'g-.','LineWidth',.5, 'Marker','o','MarkerEdgeColor', 'k','MarkerSize', 12,'MarkerFaceColor',[1.00,0.41,0.16],'MarkerIndices',marker_space);
% ylim([-1 1])
% xlim([0 SIM_TIME])
% legend('$p_y$','$p_{yr}$', ...
%     'FontSize',48, 'interpreter', 'latex', ...
...
% grid off;
% box("off")
% print('./plots/restricted_control_pert/Position2', '-depsc2')
% hold off
...
% set(gcf,"Position",[80,220,1850,525])
% set(gcf,"Renderer","painters")
% plot(time,plot_desired_position(3, :), 'b','LineWidth',5);
% hold on
% plot(time,plot_position(3, :), 'b-.','LineWidth',.5, 'Marker','o','MarkerEdgeColor', 'k','MarkerSize', 12,'MarkerFaceColor',[0.40,0.90,0.05],'MarkerIndices',marker_space);
% ylim([-1 1])
% xlim([0 SIM_TIME])
% legend('$p_z$','$p_{zr}$', ...
%     'FontSize',48, 'interpreter', 'latex', ...
...
% grid off;
% box("off")
% print('./plots/restricted_control_pert/Position3', '-depsc2')
% hold off
...
% grid off;
% box("off")
% print('./plots/restricted_control_pert/Constraint', '-depsc2')
% hold off
% %
% figure;
% set(gcf,"Color",[1,1,1])
...
% grid off;
% box("off")
% print('./plots/restricted_control_pert/tkp', '-depsc2')
% hold off
...
% set(gca,"FontSize", 30);
% xlabel('Time [s]','FontSize',40);
% ylabel('Trans kd','FontSize',40);
% grid off;
% box("off")
% print('./plots/restricted_control_pert/tkd', '-depsc2')
% hold off
...
% grid off;
% box("off")
% print('./plots/restricted_control_pert/torques', '-depsc2')
% hold off
...
% grid off;
% box("off")
% print('./plots/restricted_control_pert/thrust', '-depsc2')
% hold off